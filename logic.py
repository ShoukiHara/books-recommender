...
def classify_student_layer(api_key, student_info):
    ...
    try:
        ...
        レイヤー定義:
        1: 初学・高1レベル（京大志望の高1生が取り組むべき、基礎〜標準レベル）
        2: 標準・高2レベル（京大志望の高2生が取り組むべき、標準〜応用レベル）
        3: 上位・高3レベル（京大志望の高3生・受験生が取り組むべき、実戦・過去問レベル）
...
def generate_book_guide(api_key, title, reviews_df):
    ...
    try:
        ...
        prompt = f'''
        参考書「{title}」について、複数の講師から寄せられた以下のレビューを元に、
        この参考書の特徴とおすすめの使い方を200文字程度で簡潔に要約してください。
        読者は大人の学習者です。論理的で硬派な文章（である・だ調）で記述してください。
